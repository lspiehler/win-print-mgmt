...
var findMatch = function(search, property) {
    ...
    let matches = 0;
    //console.log(compop);
    let searchkeys = Object.keys(search.properties)
    for(let i = 0; i <= searchkeys.length - 1; i++) {
        ...
        if(search.properties[searchkeys[i]].charAt(0) == '"' && search.properties[searchkeys[i]].charAt(search.properties[searchkeys[i]] - 1) == '"') {
            //console.log(property[searchkeys[i]].split('"')[1].toUpperCase());
            //console.log(search.properties[searchkeys[i]].toUpperCase());
            if(property[searchkeys[i]].toUpperCase() == search.properties[searchkeys[i]].split('"')[1].toUpperCase()) {
...
module.exports = function() {
    ...
    var processResponses = function() {
        ...
        for(let i = 0; i <= responses.length - 1; i++) {
            ...
            if(params.search.comparison == 'AND' && params.search.properties.hasOwnProperty('ClientId') && params.search.properties.ClientId.charAt(0) == '"' && params.search.properties.ClientId.charAt(params.search.properties.ClientId.length - 1) == '"') {
                //console.log('go instasearch!!!');
                if(response.data.hasOwnProperty(params.search.properties['ClientId'].split('"')[1])) {
                    data[params.search.properties['ClientId'].split('"')[1]] = response.data[params.search.properties['ClientId'].split('"')[1]];
                }
            } else {
                data = response.data;
            }
            ...
            for(let j = 0; j <= keys.length - 1; j++) {
                for(let k = 0; k <= data[keys[j]].length - 1; k++) {
                    if(findMatch(params.search, data[keys[j]][k])) {
                        if(params.combine) {
